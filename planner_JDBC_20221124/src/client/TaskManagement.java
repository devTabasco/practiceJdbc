...
public class TaskManagement {
	...
	public Object taskController(String clientData) {
		...
		case "12": case "13": /* StartDate만 보내기 */
	...
	private Object getTaskListCtl(String clientData) {
		...
		return !serverData.equals("")? this.makeTodoList(this.convertToArray(serverData.split(":")),clientData) + "/" + serverData: "\n   등록된 일정이 없습니다.\n";
	}
	...
	private String makeTodoList(String[][] toDoList, String clientData) {
		/* 항목 정렬 */
		...
		if(clientData.split("&")[0].split("=")[1].equals("12")) {
			...
			//1차원 배열은 갯수, 2차원배열은 데이터
			now = clientData.split("&")[1].split("=")[1];
			...
			for(int itemIdx=0; itemIdx < itemCount; itemIdx++) {
				buffer.append("     "+ (itemIdx+1) + ".   " + toDoList[itemIdx][1].substring(8, 12));
				buffer.append(" " + toDoList[itemIdx][3] + "\n");
				buffer.append("     └─   " + this.formattedDate(toDoList[itemIdx][1]) + "\t" );
				buffer.append(this.formattedDate(toDoList[itemIdx][2]) + "\t");
				buffer.append(this.processingValue(toDoList[itemIdx][4]) + "\t");
	...
	/* 1차원 배열 --> 2차원 배열화 */
	private String[][] convertToArray(String[] record){
